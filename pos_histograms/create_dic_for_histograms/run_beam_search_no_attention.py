...
from decoding_strategist_visualizations.beam_search.beam_search_pack_utils_no_attention import beam_search_decode
...
def caption_image_beam_search(encoder, decoder, image, word_map, rev_word_map, beam_size):
    ...
    idx_count = 0
    for w, token, score, exp, logit in zip(words, doc, top_seq_total, ex_top_seq_total, logits_list):
        if args.debug:
    ...
    return seq, _, top_seq_total_scors, seq_sum, words
