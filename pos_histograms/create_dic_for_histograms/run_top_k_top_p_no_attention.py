...
from decoding_strategist_visualizations.top_k_top_p_captions.top_k_p_pack_utils_no_attention import caption_image
from utils import *
...
def caption_image_beam_search(encoder, decoder, image, word_map, rev_word_map):
    seq, _, seqs_prop, seqs_logits = caption_image(encoder, decoder, image, word_map, top_k, top_p, device)
    ...
    idx_count = 0
    for w, token, prop, logit in zip(words, doc, seqs_prop, seqs_logits):
    ...
    return seq, None, seqs_prop, sen_likelihood, words
...
if __name__ == '__main__':
    ...
    if args.data == 'random':
        ...
        for i in tqdm(range(args.random_range)):
            ...
            _, _, _, sen_likelihood, _ = caption_image_beam_search(encoder, decoder, image, word_map, rev_word_map)
            sentences_likelihood.append(sen_likelihood)
