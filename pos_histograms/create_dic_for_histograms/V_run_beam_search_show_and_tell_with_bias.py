...
def get_models(model_path, device, vocab_size):
    ...
    print('representations requires_grad: {}'.format(representations.requires_grad))

    return encoder, decoder, representations, checkpoint['bias']
...
def beam_search_decode(encoder_, decoder_, beam_size_, word_map_, image_, args_, representations_, bias_):
...
def caption_image_beam_search(encoder_, decoder_, image_, word_map_, rev_word_map_, args_, representations_, bias_):
    ...
    seq_, top_seq_total_scors_, seq_sum_, logits_list_ = beam_search_decode(encoder_, decoder_, beam_size, word_map_,
                                                                            image_, args_, representations_, bias_)
...
if __name__ == '__main__':
    ...
    encoder, decoder, representations, bias = get_models(model_path, device, len(word_map))
    ...
    for bi, (image, caps, caplens, allcaps) in tqdm(enumerator):
        ...
        seq_, top_seq_total_scors_, seq_sum_, words = caption_image_beam_search(encoder, decoder, image, word_map,
                                                                                rev_word_map, args, representations, bias)
        if not None == words:
    ...
    # section: save dic
    dic_name = 'pos_dic_test_beam_{}'.format( args.beam_size)
    ...
    metrics_result_file_name = 'metrics_results_test_beam_{}'.format( args.beam_size)
...
# V_run_beam_search_show_and_tell_with_bias.py
