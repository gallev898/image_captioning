...
batch_size = 3 if args.run_local else 32
...
def main():
    ...
    embeddings = get_embeddings(EMBEDDING_SIZE, len(word_map))
    embeddings = embeddings.clone().to(device).requires_grad_(False)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, embeddings):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        # Clip gradients - נשים זב זה מונע אקספלודינג גרדיאנט ולא ואנישינג!
...
def validate(val_loader, encoder, decoder, criterion, rev_word_map, embeddings):
    """
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            scores, prenorm_scores, caps_sorted, decode_lengths, alphas, sort_ind = decoder(imgs, caps, caplens)
