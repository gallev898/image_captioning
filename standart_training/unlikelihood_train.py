...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        fake_caps_and_lens_lst = random.sample(list(train_caps_lst), args.batch_size)
        ...
        targets = caps[:, 1:]
        ones = torch.ones([args.batch_size, scores.shape[1], scores.shape[2]])
        minus_ones = torch.ones([args.batch_size, scores.shape[1], scores.shape[2]]).fill_(-1)
        unlikelihood = torch.cat((ones, minus_ones)).to(device)
        scores = torch.mul(scores, unlikelihood)
